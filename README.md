...
- **A record cache**, at `~/Library/Containers/com.tokentab.TokenTab/Data/Library/Caches/TokenTab/record-cache-v4.jsonl`
  ...
  parser produces, which has no field for your text. It does not even keep the message
  and request ids: those exist only to recognise a repeated record, so each pair is
  reduced to a single 64-bit number and the ids themselves are discarded. Note that the
  paths are Claude Code's project directory names, which encode the directories you
  work in.
