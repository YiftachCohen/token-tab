...
| 2026-08-02 | **Codex window meaning comes from duration, not raw slot** | OpenAI can emit a weekly-only `10080`-minute allowance in `rate_limits.primary`. Classify `300` minutes as 5h and `10080` as weekly; when only weekly exists, the Codex panel labels that gauge as weekly and includes the reset weekday. A weekly percentage never competes in the max-pressure rule against Claude's 5h percentage |
| 2026-08-04 | **A provider's own pair shows that provider's own window; only the headline is a comparison** | The dual label's Codex pair displays whichever official window Codex published — 5h when there is one, else the weekly-only allowance — exactly like the Codex panel and secondary row. Restricting it to the 5h reading (as the max-pressure rule rightly does) dropped every weekly-only Mac's bar to a raw token count while the dropdown one click away showed a percentage for the same provider. The single `.headline` figure stays 5h-only: that one IS the comparison against Claude's 5h %, and it's what the SwiftBar label mirrors |
| 2026-08-04 | **A menu-bar figure never truncates; the item's width follows the label** | The status item's `length` is what gives the hosted label its width, so a stale `length` doesn't render as a narrow item — SwiftUI resolves the shortfall by truncating a figure, and `92%` becomes `92…`, a number missing its unit. So: the trailing pin to the button is low-priority and the label is `.fixedSize()` (it can never be squeezed into an ellipsis), the width is measured from the hosted view's OWN ideal size rather than a fitting size the too-narrow item could confirm, and a SwiftUI content-size invalidation — not a store publish — is what re-drives `length`, since a publish that arrives before the view updates leaves the item stuck narrow indefinitely |
| 2026-08-15 | **A near-limit Claude weekly allowance can headline** | Claude enforces both periods, so a 97%-used week must not hide behind a reassuring 4%-used session. When both fresh readings exist, weekly competes only at **70% used or higher** and only when it is more used than the session; below that floor it remains detail so an ordinary mid-week percentage does not replace the actionable 5-hour pace. A lone weekly reading remains authoritative. Weekly menu-bar figures carry a compact `wk` suffix, the native and SwiftBar labels use the same rule, and the weekly detail cell is suppressed while that allowance owns the hero. This supersedes the earlier 5h-only headline clause for Claude only; Codex weekly remains excluded from cross-provider pressure comparison. Explicitly approved after the reported 97%-weekly / 4%-session case. |
| 2026-08-16 | **A learned cap expires with the reading that taught it** | The calibrated cap is `windowTokens / sessionPct` — a token count over a *model-weighted* server percentage — so it is only true while that model mix is. Persisted with no age, it outlived its evidence: a 46-hour live outage left the hero reading "78% left" against a true 42%, because an all-Opus window was being measured with a ratio learned on a cheaper mix. So the cap now carries the timestamp it was learned at and leaves the precedence chain after **24 hours** — long enough to ride out an overnight sleep or a missed helper run, short enough that one reading can't underwrite a percentage forever. Past that the ring falls back to the time countdown it already shows with no cap, captioned as the clock rather than usage, and the stale-live row states the consequence instead of leaving the swap to a caption change. The same rule as the 2026-07-31 expired-window row, applied to the other direction of staleness: ours, not the provider's. A manual or configured cap is stated intent, not inference, and never expires |
